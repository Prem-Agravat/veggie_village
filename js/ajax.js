$(document).ready(function(){
	$('#submit_reg').click(function(){

		var name = $('#name_reg').val()
		var email = $('#email_reg').val()
		var password = $('#password_reg').val()
		var con_password = $('#con_password_reg').val()

		var mail_regex = /^([a-zA-Z0-9_.+-])+\@(([a-zA-Z0-9-])+\.)+([a-zA-Z]{2,4})+$/;

		var name_regex = /^[(A-Z)?(a-z)?(0-9)?\s*]+$/;

		if ((name == "") || (email == "") || (password == "") || (con_password == "")) {

			$('#reg_error').text("Don't leave the fields blank!");
		}
		else if (!mail_regex.test(email)) {

			$('#reg_error').text('Enter a valid Email!');
		}
		else if (!name_regex.test(name)) {

			$('#reg_error').text('Enter a Proper Name!');
		}
		else if (password != con_password) {

			$('#reg_error').text("Passwords doesn't match!");
		} else {

			$.ajax({
	            url :'/veggie_village/backends/register.php',
	            type:'POST',
	            data :{
	            'name':name,
	            'email':email,
	            'password':password
	            },
	            dataType:'json',
	            beforeSend:function(){
	                // $('#my_data').html('<img src="images/ajax-loader.gif" alt="Loading...">');
	                $('#submit_reg').prop("disabled", true);

	            },
	            success  :function(data){

	            	$('#name_reg').val("")

	            	$('#email_reg').val("")

	            	$('#password_reg').val("")

	            	$('#con_password_reg').val("")

	            	var instance2 = M.Modal.getInstance($('#modal2'));

	            	instance2.close();

	            	if (data['code'] == "0") {

	            		toggleModal('Error!', data['msg']);

	            	} else if (data['code'] == "1") {

	            		toggleModal('Success!', data['msg']);

	            	}
	            	$('#submit_reg').prop("disabled", false);

	           }
	        });
		}
	})

	$('#login_btn').click(function(){
		var email = $('#email_login').val()
		var password = $('#password_login').val()

		var mail_regex = /^([a-zA-Z0-9_.+-])+\@(([a-zA-Z0-9-])+\.)+([a-zA-Z]{2,4})+$/;

		if ((email == "") || (password == "")) {

			$('#login_error').text("Don't leave the fields blank!");
		}
		else if (!mail_regex.test(email)) {

			$('#login_error').text('Enter a valid Email!');
		} else {

			$.ajax({
	            url :'/veggie_village/backends/login.php',
	            type:'POST',
	            data :{
	            'email':email,
	            'password':password
	            },
	            dataType:'json',
	            beforeSend:function(){
	                // $('#my_data').html('<img src="images/ajax-loader.gif" alt="Loading...">');
	                $('#login_btn').prop("disabled", true);

	            },
	            success  :function(data){
	            	$('#email_reg').val("")

	            	$('#password_reg').val("")

	            	var instance2 = M.Modal.getInstance($('#modal1'));

	            	instance2.close();

	            	if (data['code'] == "0") {

	            		toggleModal('Error!', data['msg']);

	            	} else if (data['code'] == "1") {

	            		// toggleModal('Success!', data['msg']);

	            		location.reload(true);

	            	}
	            	$('#login_btn').prop("disabled", false);

	           }
	        });


		}
	});

});

function isEmail(evt) {
    var charCode = evt.which || event.charCode || event.char;
    if ((charCode < 65 || charCode > 90) && (charCode < 97 || charCode > 122) && (charCode != 64) && (charCode != 46) && (charCode != 95) && (charCode != 33) && (charCode > 31 && (charCode < 48 || charCode > 57)))
        return false;

    return true;
}

function isAlphaNumSpace(evt) {
    var charCode = evt.which || event.charCode || event.char;
    if ((charCode < 65 || charCode > 90) && (charCode < 97 || charCode > 122) && (charCode != 64) && (charCode != 46) && (charCode != 95) && (charCode != 33) && (charCode > 31 && (charCode < 48 || charCode > 57)) && (charCode != 32))
        return false;


    return true;
}
let price = $("#price").text();
function updateTotalPrice() {
	let quantity = Number($("#quantity").val());
	let totalPrice = quantity * Number(price);
	$("#price").text(totalPrice);
	$("#price2").val(totalPrice);
	$('#offerdiv').css('display',"inline");
	$('.msg1').fadeIn(3000); 
}

function increaseQuantity() {
	$("#quntity_add").css("color", "red");
	let input = $("#quantity").val();
	if(input == 100){
		return false;
	}else{
		$("#quantity").val(Number(input)+1);
		updateTotalPrice();
	}
}

function decreaseQuantity() {
	let input = $("#quantity").val();
	if(input == 1){
		return false;
	}else{
		$("#quantity").val(Number(input)-1);
		updateTotalPrice();
	}
}

function codefunction(){
	var select = $('select[name="code"]').val();
	var input = $("#promo_code").val();
	var price = $("#price2").val();
	var discount = $('select[name="code"]').find(':selected').data('discount');
	var formula = price*(discount/100);
	if(select == input){
		var store2 = Number($("#price").text())-formula;
		$("#price").text(store2);
		$('#msg1').fadeIn(3000); 
        $('#msg1').fadeOut(3000);
		$('#offerdiv').css('display','none');
		$("#promo_code").val("");
		$("#offer").val($('select[name="code"]').val());
		$("#price2").val(store2);
	}else{
		$("#codelable").text("Invalid Code!");
		$("#offer").val("");
	}
}
